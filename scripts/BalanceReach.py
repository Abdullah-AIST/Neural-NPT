...
posTraj = np.load('./scripts/sysID/jointPosAll.npy').reshape(-1,trajLen,6)
velTraj = np.load('./scripts/sysID/jointVelAll.npy').reshape(-1,trajLen,6)
accTraj = np.load('./scripts/sysID/jointAccAll.npy').reshape(-1,trajLen,6)
...
for traj_id in range(n):
    ...
    np.savetxt(f'./data/BR/real/trajID{traj_id}_position_velocity_real_BR_{control_freq}Hz.csv', Data, delimiter=',', header='Time,Actual Position,Desired Position,Actual Velocity', comments='')
